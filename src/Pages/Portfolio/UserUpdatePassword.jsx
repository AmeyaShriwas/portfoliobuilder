...
const UserUpdatePassword = ({ isMobile, setIsMobile }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.newPassword.trim()) errors.newPassword = "Password is required";
  ...
  const handlePasswordSubmit = (e) => {
    e.preventDefault();
    if (validateForm()) {
      dispatch(updatePassword(formData)).then((response) => {
      ...
      >
        ...
        >
          ...
          <div className="text-center mt-3">
            ...
            <span className="text-dark" style={{ cursor: "pointer" }} onClick={() => navigate("/user/login")}>
              Login now
            </span>
          </div>
        </motion.div>
